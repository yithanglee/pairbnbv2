class PaymentsController < ApplicationController
  ...
  def create
    ...
    @booking = Reservation.find(params[:payment][:reservation_id])
    ...
    if @result.success?
      Payment.create(reservation_id: params[:payment][:reservation_id], braintree_payment_id: @result.transaction.id, status: @result.transaction.status, fourdigit: @result.transaction.credit_card_details.last_4)
        ...
        redirect_to listing_reservation_path(@booking.listing.id , @booking.id ), notice: "Congratulations! Your transaction is successful!"
    else
